...
interface EditProgramFormProps {
  program: Program;
  onSubmit: (data: EditProgramFormData) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
}

export function EditProgramForm({ program, onSubmit, onCancel, isLoading = false }: EditProgramFormProps) {
  ...
  const handleFormSubmit = async (data: EditProgramFormData) => {
    try {
      setErrors({});
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 sm:gap-3 pt-2 sm:pt-3 md:pt-4">
        ...
        <Button type="submit" disabled={isSubmitting || isLoading} className="h-8 sm:h-9 text-xs sm:text-sm">
          {isSubmitting || isLoading ? "Updating..." : "Update Program"}
        </Button>
      </div>
    </form>
  );
